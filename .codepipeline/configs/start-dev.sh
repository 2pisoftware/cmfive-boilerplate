...
echo "zend_extension=xdebug.so" > /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
	&& echo "[xdebug]" >> /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
	&& echo "xdebug.remote_enable=1" >> /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
	&& echo "xdebug.remote_host=${INTERNAL_HOST_IP}" >> /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
	...
	&& echo "xdebug.remote_handler=dbgp" >> /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
	&& echo "xdebug.max_nesting_level=250" >> /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
	&& echo "xdebug.client_host=${INTERNAL_HOST_IP}" >> /etc/php/7.4/fpm/conf.d/20-xdebug.ini \
