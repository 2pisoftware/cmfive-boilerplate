...
export class TimelogHelper  {
    static async createTimelogFromTimer(page: Page, timelog: string, taskName: string, start_time: string = "")
    {
    ...
    static async createTimelog(page: Page, timelog: string, taskName: string, date: DateTime, start_time: string, end_time: string)
    {
        await CmfiveHelper.clickCmfiveNavbar(page, "Task", "Task List");
        await page.getByRole("link", {name: taskName, exact: true}).click();
    ...
    static async editTimelog(page: Page, timelog: string, taskName: string, date: DateTime, start_time: string, end_time: string)
    {
        await CmfiveHelper.clickCmfiveNavbar(page, "Task", "Task List");
        await page.getByRole("link", {name: taskName, exact: true}).click();
        await page.getByRole("link", {name: "Time Log"}).click();
    ...
    static async deleteTimelog(page: Page, timelog: string, taskName: string)
    {
        await CmfiveHelper.clickCmfiveNavbar(page, "Task", "Task List");
        await page.getByRole("link", {name: taskName, exact: true}).click();
        await page.getByRole("link", {name: "Time Log"}).click();
